...
public:
        ...
        int64_t numLocalExperts, int64_t numTokens) const
    {
        ...
        // The non-fused tileN=8 dynB cubins hit the same GEMM2 TMA fault during
        // CUDA graph replay for Laguna FP8 on B200. TileN=16 is unaffected.
        // TLLM_MOE_MIN_TILEN overrides the threshold (0 disables) for A/B experiments.
        static int const minTileN = []()
        {
            char const* env = std::getenv("TLLM_MOE_MIN_TILEN");
        ...
        {
            int const requiredMinTileN = numFusedSharedExpert.value_or(0) > 0 ? fusedMinTileN : minTileN;
            if (tileN < requiredMinTileN)
            {
                continue;
            }
        ...
        std::optional<at::Tensor> const& output = std::nullopt)
    {
        ...
        if (tileN == -1 || config == -1)
        {
            ...
            {
                static int const minTileNFallback = []()
                {
                    char const* env = std::getenv("TLLM_MOE_MIN_TILEN");
                    return env != nullptr ? std::atoi(env) : 16;
                }();
                tileN = std::max<int64_t>(tileN, minTileNFallback);
